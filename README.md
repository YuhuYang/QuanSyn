...
- **Sentence-level**: mdd, ndd, mhd, mhdd, tdl, sl, mv, vk, tw, th, hi, hf, rd  
    ...
    - rd: Root Distance, the linear position of the root in the sentence (1-based index).  
    ...
    - ndd: Normalized Dependency Distance, defined as $|\log(\frac{mdd}{\sqrt{rd\cdot sl}})|$.  
        - Lei, L., & Jockers, M. L. (2020). Normalized Dependency Distance: Proposing a New Measure. Journal of Quantitative Linguistics, 27(1), 62–79. 
...
- **Text-level**: mdd, ndd, mhd, mhdd, mtdl, msl, mv, vk, mtw, mth, hi, hf, mrd  
    ...
    - mrd: Mean Root Distance, the average root position across all sentences in the text.  
    - ndd: Mean Normalized Dependency Distance across all sentences in the text.  
    - hi: Proportion of head-initial dependencies in the text.  
    - hf: Proportion of head-final dependencies in the text.

- **Distributions**: dd, hd, v, sl, tw, th, rd, pos, deprel  
...
# Perform the calculation (default: serial)
...
# DepValAnalyzer also accepts a parsed sentence list or a raw conllu string.
# Use cleaned=True if your input is already preprocessed.
...
```
The return format is a dictionary `dict`, where the `key` indicates governor/dependent role and the `value` is a list of `(label, probability)` pairs.
